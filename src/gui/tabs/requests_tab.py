...
from src.constants import MAIN_LAYOUT_MARGINS, WIDGET_SPACING
...
class RequestDetailsPanel(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self._current_request: FOIARequest | None = None
        self._setup_ui()
    ...
    def _setup_ui(self):
        ...
        self.foia_text_edit.textChanged.connect(self._on_field_changed)
        self.status_combo.currentTextChanged.connect(self._on_field_changed)
        self.deadline_edit.dateChanged.connect(self._on_field_changed)

    def set_request(self, request: FOIARequest | None):
        ...
        if request:
            ...
            self.stats_labels["responsive"].setText(
                f"Responsive: {summary['responsive']}"
            )
            self.stats_labels["non_responsive"].setText(
                f"Non-Responsive: {summary['non_responsive']}"
            )
            self.stats_labels["uncertain"].setText(f"Uncertain: {summary['uncertain']}")

            self.save_button.setEnabled(False)  # Disabled until user makes changes
        else:
            # Clear all fields
    ...
    def get_updated_values(self) -> dict:
        ...
        return {
            ...
            "deadline": datetime.combine(
                self.deadline_edit.date().toPyDate(), datetime.min.time()
            ),
        }
...
class RequestsTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Header with title and active request
        ...
        layout.addWidget(splitter, 1)  # Add stretch factor to fill space
    ...
    def _refresh_table(self):
        ...
        for row, request in enumerate(requests):
            ...
            # Deadline
            deadline_str = (
                request.deadline.strftime("%Y-%m-%d") if request.deadline else "-"
            )
            self.request_table.setItem(row, 5, QTableWidgetItem(deadline_str))

            # Store request ID in first column
            self.request_table.item(row, 0).setData(
                Qt.ItemDataRole.UserRole, request.id
            )
    ...
    def _on_new_request(self):
        ...
        if ok and name:
            ...
            for row in range(self.request_table.rowCount()):
                if (
                    self.request_table.item(row, 0).data(Qt.ItemDataRole.UserRole)
                    == request.id
                ):
    ...
    def _on_save_changes(self):
        ...
        if self.request_manager.update_request(request_id, **values):
            self._refresh_table()
            QMessageBox.information(self, "Success", "Request updated successfully.")
            self.details_panel.save_button.setEnabled(False)
        else:
            QMessageBox.warning(self, "Error", "Failed to update request.")

    def _update_active_request_display(self):
        """Update the active request display."""
        if hasattr(self, "_active_request_label"):
            active_request = self.request_manager.get_active_request()
            if active_request:
                self._active_request_label.setText(f"Active: {active_request.name}")
            else:
